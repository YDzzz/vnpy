...
from model.vnpy_LongPort.longbridge_gateway import convert_symbol_vt2lb, INTERVAL_MAP, convert_candlestick_bar
from model.vnpy_datamanager.mysql_database import get_database
...
EXCHANGE_VT2UDATA = {
    Exchange.SMART: str(Market.US),
    Exchange.SEHK: str(Market.HK)
}
...
class MyDatafeed(BaseDatafeed):
    ...
    def query_tick_history(self, req: HistoryRequest, output: Callable = print) -> Optional[List[TickData]]:
        ...
        return [tick]
        # v_symbol = convert_symbol(req.symbol, EXCHANGE_VT2UDATA.get(req.exchange))
        # depth = self.quote_ctx.realtime_depth(v_symbol)
        # tick = TickData(
        #     symbol=req.symbol,
...
datafeed: Union[MyDatafeed, None] = None
...
if __name__ == '__main__':
    fd = get_datafeed()
    req = HistoryRequest('3918', Exchange.SEHK, datetime(2024, 11, 21), datetime(2024, 11, 22), Interval.MINUTE)
    db = get_database()

    db.save_bar_data(fd.query_bar_history(req))
