...
from model.vnpy_spreadtrading.base import EVENT_SPREAD_DATA, SpreadItem
...
class RecorderEngine(BaseEngine):
    ...
    def update_tick(self, tick: TickData) -> None:
        """"""
        # 过滤偏离本地时间戳过大的Tick数据
        tick_datetime = tick.datetime.replace(tzinfo=None)
        filter_datetime = self.filter_dt.replace(tzinfo=None)
        tick_delta: timedelta = abs(tick_datetime - filter_datetime)
