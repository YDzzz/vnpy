...
from longport.openapi import QuoteContext, TradeContext, Config, Market, SubType, PushQuote, PushTrades, Period, \
    ...
    PushCandlestick, TradeSession, Candlestick, OpenApiException
...
def build_config_from_setting() -> Config:
    ...
    return Config(app_key=SETTINGS['longbridge.app_key'],
                  app_secret=SETTINGS['longbridge.app_secret'],
                  access_token=SETTINGS['longbridge.access_token']
                  )


class LongBridgeGateway(BaseGateway):
    default_name = "LongBridge"
    exchanges = [Exchange.SMART, Exchange.SEHK]
    ...
    def handle_quote(self, symbol: str, quote: Union[PushQuote, SecurityQuote]):
        ...
        tick = TickData(
            ...
            limit_up=float(quote.last_done)+0.1,
            limit_down=float(quote.last_done)-0.1,
    ...
    def subscribe_batch(self, reqs: List[SubscribeRequest], contract_only=False) -> None:
        ...
        for info in self.quote_ctx.static_info(symbols):
            ...
            self.symbol_names[info.symbol] = info.name_cn
        if contract_only:
            return
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        try:
            ...
            if e.code == 602001:
                raise Exception(f'req error:{req}')
        else:
            self.write_log(f"order of {symbol} submitted, order_id={resp.order_id}")
            return self.gateway_name+'.'+resp.order_id
